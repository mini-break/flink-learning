...
import com.zhisheng.function.CustomSource2;
...
public class Main2_new2 {
    public static void main(String[] args) throws Exception {
        ...
        // 增加水位线
        DataStream<WordEvent> data = env.addSource(new CustomSource2()) // 加入自定义数据源
                .assignTimestampsAndWatermarks(((WatermarkStrategy<WordEvent>) context -> new WatermarkGenerator<WordEvent>() {
                    /**
                ...
                }).withTimestampAssigner((SerializableTimestampAssigner<WordEvent>) (element, recordTimestamp) -> { // 获取业务时间戳
                    long timestamp = element.getTimestamp();
                    currentTimestamp[0] = Math.max(timestamp, currentTimestamp[0]);
                    System.out.println("timestamp:" + timestamp + " currentTimestamp:" + currentTimestamp[0]);
                    return timestamp;
                }));
//        data.print();
